...
const PDF_URL = 'https://api.syfre.ai/wp-content/uploads/2026/04/Luanda-Presentation-Final.pdf';

export default function LuandaPresentationForm() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const leadData = {
      ...
      Lead_Source: 'ACI Africa 2026 - Luanda Presentation Download',
    };
  ...
  if (submitStatus === 'success') {
    return (
      <div className="bg-zinc-900 border border-zinc-800 rounded-2xl px-6 sm:px-10 py-10 sm:py-12 text-center">
        ...
        <h3 className="text-2xl font-semibold text-white mb-3">Your presentation is ready</h3>
        <p className="text-zinc-400 text-base leading-relaxed max-w-sm mx-auto mb-7">
          Click below to open the full PDF. It covers everything from the ACI Africa session in Luanda.
        </p>
        <a
          href={PDF_URL}
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex items-center gap-2 bg-green-400 text-black font-semibold text-base px-6 py-3.5 rounded-lg hover:bg-green-500 transition-colors w-full justify-center"
        >
        ...
        <p className="text-zinc-600 text-xs mt-5">
          Tim will also be in touch if you&apos;d like to continue the conversation.
        </p>
      </div>
    );
  }
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-5">
        ...
        <div className="flex flex-col gap-1.5">
          ...
          >
            ...
            <option value="Pre-deployment readiness">Pre-deployment readiness</option>
        ...
        >
          {isSubmitting ? 'One moment...' : 'Send me the PDF'}
        </button>

        <p className="text-zinc-600 text-xs text-center">
          Free. No obligation. Instant access.
        </p>
      </form>
    </div>
  );
